...
def fit_drdi(x, y, y_err, *, min_value=1e-2, max_value=2.4, trim_edges=False, min_points=3):
    """Straight-line fit: y = p0 + p1*x. Nothing implicit here -- `x` is used
    exactly as given (no cumsum, no re-derived axis). It must already be the
    x-axis you plan to evaluate/plot the fit against, e.g. real elapsed
    wall-clock time in seconds.

    Which points are used:
      1. x and y must both be finite.
      2. If min_value/max_value are given, y must fall within [min_value,
         max_value] -- this is the *only* outlier rejection: an explicit,
         inspectable range cut, nothing fancier.
      3. If trim_edges=True and more than min_points points survive steps
         1-2, the first and last of those points are also dropped from the
         fit itself (kept in `good`, just not used to compute p0/p1).

    Weighting -- kept binary on purpose, no partial/imputed weighting:
      - If EVERY surviving point has a finite, positive y_err: weighted
        least squares (y_err trusted as an absolute 1-sigma uncertainty).
      - Otherwise (any point missing an error, e.g. an upstream error
        column that's all-NaN): plain unweighted least squares. u_p0/u_p1
        still come out finite, just derived from scatter around the line
        rather than from errors you don't have yet.

    Returns: p0, p1, u_p0, u_p1, chi2, pvalue, ndf, error_flag, good
    `good` is a boolean array the same length as the input, True for points
    that passed steps 1-2 (regardless of trim_edges).
    """
    x = np.asarray(x, dtype=float)
    ...
    xf, yf, ef = x[fit_mask], y[fit_mask], y_err[fit_mask]
    n = len(xf)
    if n <= 2:
        return np.nan, np.nan, np.nan, np.nan, np.nan, np.nan, n, True, good

    weighted = np.isfinite(ef).all() and (ef > 0).all()
    try:
        if weighted:
            # w = 1/sigma is what np.polyfit expects; cov='unscaled' keeps the
            # covariance on the absolute scale set by the given errors,
            # instead of rescaling it by the fit's own residual chi2.
            (p1, p0), cov = np.polyfit(xf, yf, 1, w=1.0 / ef, cov='unscaled')
            ndf = n - 2
            chi2_val = float(np.sum(((yf - (p0 + p1 * xf)) / ef) ** 2))
            pval = 1 - chi2.cdf(chi2_val, ndf) if ndf > 0 else np.nan
        else:
            # No (or incomplete) error info -> plain unweighted OLS. chi2/pval
            # aren't meaningful without real uncertainties, so leave them NaN.
            (p1, p0), cov = np.polyfit(xf, yf, 1, cov=True)
            ndf, chi2_val, pval = n - 2, np.nan, np.nan
        u_p1, u_p0 = np.sqrt(cov[0, 0]), np.sqrt(cov[1, 1])
